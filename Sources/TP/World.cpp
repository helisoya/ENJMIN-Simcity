...
void World::Generate(DeviceResources* deviceRes)
{
	...
	siv::BasicPerlinNoise<float> noise(786768768876);

	for (int z = 0; z < MAP_SIZE; z++) {
		for (int x = 0; x < MAP_SIZE; x++) {
			GenerateChunck(Vector3(CHUNCK_SIZE * x, 0, CHUNCK_SIZE * z),noise, deviceRes);
		}
	}
...
void World::GenerateFromFile(DeviceResources* deviceRes, std::wstring filePath)
{
    ...
    SetupEmptyChunks(deviceRes);


    std::fstream fin;
    std::wstring file = std::wstring(L"Tilemap/") + filePath + L".csv";
    //file = L"C:/Users/oudot/Documents/GitHub/ENJMIN-Simcity/Resources/Tilemap/TestMap.csv";
    ...
    while (getline(fin, line))
    {
        ...
        while (getline(s, word, ','))
        {
            ...
            xInChunck = x % CHUNCK_SIZE;
            yInChunck = y  % CHUNCK_SIZE;

            Chunck& chunck = chuncks.at(To1D( std::floorf((float)x / (float)CHUNCK_SIZE), std::floorf((float)y / (float)CHUNCK_SIZE)));

            chunck.SetChunckData(Vector3(xInChunck, 0, yInChunck), dataSand);
            ...
            for (int up = 1; up <= yMax; up++) {
                chunck.SetChunckData(Vector3(xInChunck, up, yInChunck), up < 3 ? dataGrass : dataMountain);
            }
...
void World::SetupEmptyChunks(DeviceResources* deviceRes)
{
    for (int z = 0; z < MAP_SIZE; z++) {
        for (int x = 0; x < MAP_SIZE; x++) {
            int id = chuncks.size();
            chuncks.emplace_back(Vector3(CHUNCK_SIZE * x,0,CHUNCK_SIZE * z), id);
        }
    }
}

void World::GenerateChunck(Vector3 pos, siv::BasicPerlinNoise<float>& noise, DeviceResources* deviceRes)
{
	int id = chuncks.size();
	...
	float noiseValue;
	int yMax;
	float mapSize = MAP_SIZE * CHUNCK_SIZE / 2;
		...
		for (int z = 0; z < CHUNCK_SIZE; z++) {
			...
			for (int y = 0; y < yMax && y < 7; y++) {
                ...
                if (y == 0) {
                    ...
                    data = BlockData::Get(y < 3 ? GRASS : STONE);
                }
